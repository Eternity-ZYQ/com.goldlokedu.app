...
'获取课程表时间'
WS.sendRequestAndVerify(findTestObject("Object Repository/Api/Mobile Api/Campus/Course Schedule/course_schedule_time"), FailureHandling.CONTINUE_ON_FAILURE)
...
if(class_jsonResponse.data.size>0){
	...
	for(int x:(0..class_jsonResponse.data.size-1)){
		...
		WS.comment('加载'+class_name+'课程表中...')
		
		WS.sendRequestAndVerify(findTestObject('Object Repository/Api/Mobile Api/Campus/Course Schedule/class_course_schedule', [('class_id'):class_id]), FailureHandling.CONTINUE_ON_FAILURE)
