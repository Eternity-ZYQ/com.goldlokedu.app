			...
			if(get_album_list_jsonResponse.albums.size>0){
				...
				if(get_ablum_picture_list_jsonResponse.pictures.size>0){
					...
					for(int k:(0..get_ablum_picture_list_jsonResponse.pictures.size-1)){
						...
						'删除相册图片'
						WS.sendRequestAndVerify(findTestObject("Object Repository/Api/Mobile Api/Campus/Class Circle/Album/delete_picture",[('album_pictures'):album_pictures_list.toString(),('class_id'):class_id]), FailureHandling.CONTINUE_ON_FAILURE)
