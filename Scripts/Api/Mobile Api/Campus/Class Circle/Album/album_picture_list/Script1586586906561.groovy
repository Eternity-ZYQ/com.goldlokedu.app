		...
		if(get_album_list_jsonResponse.albums.size>0){
			...
			if(get_ablum_picture_list_jsonResponse.pictures.size>0){
				...
				for(int k:(0..get_ablum_picture_list_jsonResponse.pictures.size-1)){
					...
					'加载图片'
					dowmlaod_picture(get_ablum_picture_list_jsonResponse.pictures[k].picture_id)
				}
...
//加载图片
def void dowmlaod_picture(String picture_id){
	'获取教师关联班级信息'
	ResponseObject dowmlaod_pictureresponse=WS.sendRequest(findTestObject("Object Repository/Api/Mobile Api/Campus/Class Circle/Album/dowmload_album_picture",[('picture_id'):picture_id]), FailureHandling.CONTINUE_ON_FAILURE)
	
	WS.verifyResponseStatusCode(dowmlaod_pictureresponse, 200, FailureHandling.CONTINUE_ON_FAILURE)
