	...
	for(int y:(0..class_information_jsonResponse.data[x].klass.size-1)){
		...
		if(get_album_list_jsonResponse.albums.size>0){
			...
			if(judge_adviser_jsonResponse.data.adviser){
				...
				delete_album(class_id,get_album_list_jsonResponse.albums[0].album_id)
...
//删除相册
def void delete_album(String class_id,String album_id){
	'发送添加相册接口'
	ResponseObject delete_album_response=WS.sendRequest(findTestObject("Object Repository/Api/Mobile Api/Campus/Class Circle/Album/delete_album",[('class_id'):class_id,('album_id'):album_id]), FailureHandling.CONTINUE_ON_FAILURE)
	
	def delete_album_jsonResponse=get_jsonResponse(delete_album_response)
	WS.comment('添加相册返回数据body：'+delete_album_response.getResponseText())
	
	if(WS.verifyResponseStatusCode(delete_album_response, 200, FailureHandling.CONTINUE_ON_FAILURE)){
		
		WS.verifyElementPropertyValue(delete_album_response, 'result', 'Success', FailureHandling.CONTINUE_ON_FAILURE)
