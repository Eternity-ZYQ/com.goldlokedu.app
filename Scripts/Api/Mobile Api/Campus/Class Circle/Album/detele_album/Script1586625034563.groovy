	...
	for(int y:(0..class_information_jsonResponse.data[x].klass.size-1)){
		...
		if(get_album_list_jsonResponse.albums.size>0){
			...
			if(judge_adviser_jsonResponse.data.adviser){
				...
				'发送删除相册接口'
				WS.sendRequestAndVerify(findTestObject("Object Repository/Api/Mobile Api/Campus/Class Circle/Album/delete_album",[('class_id'):class_id,('album_id'):get_album_list_jsonResponse.albums[0].album_id]), FailureHandling.CONTINUE_ON_FAILURE)
