...
if(jsonResponse.data.size>0){
	...
	for(int x:(0..jsonResponse.data.size-1)){
		...
		if(student_list_jsonResponse.data.size>0){
			...
			'发送消息给每个班的第一个学生'
			def send_message=student_list_jsonResponse.data[0].name+'同学,你好呀!当前时间是:'+CustomKeywords.'time.SystemTime.get_system_time'()+',我给你发的消息能收到吗?'
			ResponseObject send_chat_response=WS.sendRequestAndVerify(findTestObject('Object Repository/Api/Mobile Api/Campus/Contact Parents/send_chat_message', [('send_message'):send_message,('student_id'):student_list_jsonResponse.data[0].userId]), FailureHandling.CONTINUE_ON_FAILURE)
			...
			WS.verifyElementPropertyValue(new_msg_response, 'data[0].message', send_message, FailureHandling.CONTINUE_ON_FAILURE)
