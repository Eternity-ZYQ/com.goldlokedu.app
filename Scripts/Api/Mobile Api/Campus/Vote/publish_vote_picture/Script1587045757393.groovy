...
def void set_title(){
	 ...
	 title=CustomKeywords.'time.SystemTime.get_system_time'()+'发布图片投票标题'
...
def void set_vote_options(int size){
	...
	for(int x:(0..size-1)){
		def load_picture_response=upload_picture()
		def options='{"sequential":'+x+',"content": {"content": "测试投票文本选项'+(x+1)+'","picture_id": "'+load_picture_response.picture_id+'"}}'
		list.add(options)
	  }
...
def Object upload_picture(){
	ResponseObject response=WS.sendRequest(findTestObject("Object Repository/Api/Mobile Api/Campus/Vote/upload_vote_picture"), FailureHandling.CONTINUE_ON_FAILURE)
