...
'学生筛选科目作业列表接口路径'
def path='Object Repository/Api/Mobile Api/Campus/Homework/student_homework_course_filter_list'
'加载缩略图接口路径'
...
'获取默认学生作业列表'
ResponseObject list_response=WS.sendRequestAndVerify(findTestObject(path, [('from'):from,('size'):size,('course_id'):course_id]), FailureHandling.CONTINUE_ON_FAILURE)
...
if(list_jsonResponse.data.size>0){
	...
	WS.comment('学生作业列表没有数据,帅选科目:'+course_name)
