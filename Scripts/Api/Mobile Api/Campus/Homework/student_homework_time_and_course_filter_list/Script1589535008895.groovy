...
'学生默认作业列表接口路径'
def path='Object Repository/Api/Mobile Api/Campus/Homework/student_homework_time_and_course_filter_list'
'加载缩略图接口路径'
def path1='Object Repository/Api/Mobile Api/Campus/Homework/download_picture_thumb'

'五天前日期'
def start_time=CustomKeywords.'time.SystemTime.get_start_time'(-5)
'今天日期'
def end_time=CustomKeywords.'time.SystemTime.get_future_day_time'(0)
'获取科目'
def course_id=set_course_name_and_get_id()

'获取默认学生作业列表'
ResponseObject list_response=WS.sendRequestAndVerify(findTestObject(path, [('from'):from,('size'):size,('course_id'):course_id,('from_date'):start_time,('to_date'):end_time]), FailureHandling.CONTINUE_ON_FAILURE)
def list_jsonResponse=get_jsonResponse(list_response)

if(list_jsonResponse.data.size>0){
	
	WS.comment('学生作业列表有数据,筛选科目:'+course_name)
	...
	WS.comment('学生作业列表没有数据,筛选科目:'+course_name)
