...
'发布作业'
publish_homework_text(content,class_ids,course_id)
...
def void publish_homework_text(String content,String class_ids,String course_id){
	
	WS.sendRequestAndVerify(findTestObject('Object Repository/Api/Mobile Api/Campus/Homework/publish_homework_text', [('content'):content,('class_ids'):class_ids,('course_id'):course_id]), FailureHandling.CONTINUE_ON_FAILURE)
...
def String get_homework_content(){
	...
	if(course_name!=''){
		...
		content=CustomKeywords.'time.SystemTime.get_system_time'()+'发布的'+course_name+'作业：明晚放学别走！此处无图'
	}
