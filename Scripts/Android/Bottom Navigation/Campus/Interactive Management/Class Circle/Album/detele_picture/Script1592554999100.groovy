...
if(jsonResponse.data.size>0){
	...
	if(manager_jsonResponse.data.size>0){		//是否有管理的班级
	   ...
	   '进行删除图片操作'
	   detele_picture(class_name,klass_id)
	}else{
...
//删除图片
def void detele_picture(String class_name,String klass_id){
	'发送获取相册数据接口'
	...
	'进行删除图片操作'			//若果没有相册,则接口添加一个相册,点击更多,进入里面操作,若有相册,则外面点击相册进行操作
	detele_picture_action_operate(class_name,album_list_jsonResponse,album_list_jsonResponse.albums.size==0,klass_id)
}



//删除图片步骤操作
def void detele_picture_action_operate(String class_name,LazyMap album_list_jsonResponse,boolean is_inside,String klass_id){
	'获取第一个相册的名称'
	...
	'点击删除'
	Mobile.tap(findTestObject('Object Repository/Android/Bottom Bavigation/Campus/Interactive Management/Class Circle/Album/manage_page_detele_btn'), GlobalVariable.G_short_timeout, FailureHandling.CONTINUE_ON_FAILURE)
	'验证toast:删除成功!'
