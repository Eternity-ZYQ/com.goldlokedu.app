...
//发布公告
def void publish_notice(String class_name,String klass_id){
	'发送获取最新班级公告数据接口'
	...
	'进行发布页面发布操作'
	publish_page_action_operate(class_name)
}

//发布页面步骤操作
def void publish_page_action_operate(String class_name){
	'验证发布公告页面标题:发布通知'
	Mobile.verifyElementExist(findTestObject("Object Repository/Android/Bottom Bavigation/Campus/Interactive Management/Class Circle/Notice/notice_titile_text"), GlobalVariable.G_short_timeout, FailureHandling.CONTINUE_ON_FAILURE)
	'生成发布内容'
	def content=CustomKeywords.'time.SystemTime.get_system_time'()+'Android app发布公告---'+class_name
	Mobile.setText(findTestObject('Object Repository/Android/Bottom Bavigation/Campus/Interactive Management/Class Circle/Notice/notice_content_edittext', [('text'):'请输入'+class_name+'的公告内容']), content, GlobalVariable.G_short_timeout, FailureHandling.CONTINUE_ON_FAILURE)
	'点击发布按钮'
	Mobile.tap(findTestObject('Object Repository/Android/Bottom Bavigation/Campus/Interactive Management/Class Circle/Notice/publish_page_publish_btn'), GlobalVariable.G_short_timeout, FailureHandling.CONTINUE_ON_FAILURE)
	'验证发布成功'
	CustomKeywords.'public_action.verifyToast.VerifyToastElementExistByText'('发布成功')
